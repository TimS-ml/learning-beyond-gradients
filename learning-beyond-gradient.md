...
Continual Learning 一直难以被解决，主要卡在神经网络的灾难性遗忘：学了新东西，旧能力就容易被冲掉。那如果不把目光只放在神经网络权重上，还有没有别的路？

随着 LLM agent 的流行，大家发现自己的 coding 速度和质量都可以得到很大提升。但另一个问题也一直被提出：既然模型越来越强，那么既 Pretrain、RLHF、Large-scale RL/RLVR 之后下一个范式在哪里？

我最近的一个想法是：Continual Learning 未必只发生在神经网络权重里。代码、测试、日志、回放、memory 和工具链，也可以承载一部分学习。说具体点：让 coding agent 把反馈直接写回一个可维护的软件系统。

## Heuristic Learning

在业余时间维护 [EnvPool](https://github.com/sail-sg/envpool) 的时候，我想用一个便宜一点的策略来测试游戏环境正确性，不然每次跑神经网络都很费测试资源。
...
- Breakout 迭代几次之后，构建出一个纯 Python 代码策略，直接打到理论最高分 `864`。中间过程大概是 `387 -> 507 -> 839 -> 864`：先能稳定接球，再打破球路循环，再处理高速低位球，最后修后期偏移；
- Ant 也是纯 Python 程序策略，先从 CPG/PD 节律步态开始，到 `3000+`，后来加上 residual MPC，直接上了 `6000+` 分，已经进入常见 RL baseline 的量级；
- HalfCheetah 的 staged-tree MPC 版本，5 局复测均值到了 `11836.7`；
- Atari57 我也尝试跑了一整套无人值守实验：`57 个游戏 x 2 种输入 x 3 次运行 = 342 条 coding-agent 搜索轨迹`。表现有好有坏，但在环境交互步这个口径下，中位数 HNS 在 `1M` 环境步附近已经远高于 PPO / CleanRL EnvPool PPO 的早期曲线；最终 best-input 口径下 median HNS 是 `0.83`，和这些 baseline 的 `0.80 / 0.98` 在同一档。
...
这让我意识到有一个不同寻常的事情在发生。在和 codex 交流了一阵子之后，我想把它定义为 **Heuristic Learning（HL）**：
...
过去很多 heuristic 看起来没有前途，原因常常落在维护成本上；它们本身未必太弱。coding agent 改变的是这条维护成本曲线。规则、测试、日志、memory 和 patch 原来只是散落的工程材料，现在开始可以组成一个会持续更新的 Heuristic。
...
先看环境步曲线。在完全无人工介入的批量运行里，`native_obs` 到 `1M` 步附近的 Atari median HNS 已经到 `0.32`，`ram` 是 `0.26`，明显高于图里 PPO2 / CleanRL EnvPool PPO 的早期曲线；到 `9.7M` 步附近，`native_obs` 是 `0.81`，`ram` 是 `0.59`。同一张对比里，OpenRL Benchmark 保存的 PPO2 / CleanRL EnvPool PPO median HNS 曲线到 `10M` 步大约是 `0.88 / 0.92`。
