...
- Atari57 我也尝试跑了一整套无人值守实验：`57 个游戏 x 2 种输入 x 3 次运行 = 342 条 coding-agent 搜索轨迹`。表现有好有坏，但中位数 HNS 已经接近 PPO / CleanRL EnvPool PPO 在 `10M` 环境步附近的区间。
...
在完全无人工介入的批量运行里，`native_obs` 到 `9.7M` 步附近的 Atari median HNS 是 `0.81`，`ram` 是 `0.59`。同一张对比里，OpenRL Benchmark 保存的 PPO2 / CleanRL EnvPool PPO median HNS 曲线到 `10M` 步大约是 `0.88 / 0.92`。
